...
- 收纳页顶部提供参考 DYYY 样式的聚合搜索框，可搜索六个已建立静态目录插件的功能名称和分类；未安装的插件不会出现在结果中。
- 聚合搜索目录来自开发者审核后的扫描数据，只保存功能名称和页面路径，不保存用户设置值或账号数据。
...
正常使用时不会持续扫描 `UIWindow` / `UIView`，也不拦截悬浮球、不模糊匹配普通页面文本、不接管抖音的收藏网络请求。项目保留了显式启动的开发者扫描器，用于插件升级后的目录适配；其 UI 默认隐藏。开发者扫描只在应用前台、非列表拖动期间低频运行，进入后台会暂停；聚合搜索的定位重试也会在后台立即取消。

如需重新采集目录，可在抖音的 `NSUserDefaults` 中临时设置 `DYStorageDeveloperMode=YES`，重启抖音后“开发者扫描”入口才会显示。采集完成后删除该键或设为 `NO` 即可恢复隐藏。
