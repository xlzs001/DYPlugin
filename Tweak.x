...
static void RemoveRogueWatermarks(UIView *view) {
    ...
    // 💡 暴力安全读取文字：不管它伪装成了 UILabel 还是 UIButton，强制扒出它的内容
    ...
    // 如果文字里没有，就查它的底层类名是不是叫 抖音助手 或 XUU
    if (!shouldRemove) {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (strongSelf && strongSelf.view && strongSelf.view.window) {
            ...
            // 💡 重点：增加对导航栏的扫描！这玩意儿可能藏在左上角的 NavigationBar 里！
